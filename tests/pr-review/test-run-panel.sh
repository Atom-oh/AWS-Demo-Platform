...
# 여기서 하지 않는다(5개 병렬 job 이 끝난 뒤 chair job 의 aggregate.sh 가 담당 —
...
[ ! -e "$WORK/slot/kiro-opus-L2.md" ] && [ ! -e "$WORK/slot/claude-self-L2.md" ] \
...
# (b) kiro-gpt 태그 — 모델 자체가 gpt-5.6-terra 로 kiro-cli 를 부르는지(다른 kiro-* 와 혼동 없이)
setup; mkfake kiro-cli 0 "kiro-finding"
"$SCRIPT" "$WORK/diff.txt" "$LENSES" "$WORK" kiro-gpt >/dev/null 2>&1
[ -s "$WORK/slot/kiro-gpt-L2.md" ] \
  && pass "run-panel (b) kiro-gpt tag produces kiro-gpt-L2.md" || fail "run-panel (b) kiro-gpt tag produces kiro-gpt-L2.md" "slot missing/empty"
[ ! -e "$WORK/slot/kiro-opus-L2.md" ] && [ ! -e "$WORK/slot/kiro-glm-L2.md" ] \
  && pass "run-panel (b) other kiro tags untouched" || fail "run-panel (b) other kiro tags untouched" "sibling kiro slot appeared"
