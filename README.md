...
1단계(인프라 마이그레이션)는 완료되었습니다. 2단계(Lifecycle Controller)는 **dev에 배포 완료** — 백엔드 코드(Phase 1), 기반 인프라(Phase 2: DynamoDB/IAM/SQS/ECR/Secrets), GHA OIDC→ECR 이미지 파이프라인(Phase 3), ECS/ALB/CloudFront/Route53/Cognito 런타임(Phase 4)이 모두 가동 중입니다. `api` 서비스는 `https://admin-api-dev.atomai.click/health`에서 응답하며, `worker`는 secret 주입 + config 번들링 전까지 스캐폴드(desiredCount=0) 상태입니다. 프론트엔드(3단계)는 스캐폴드 상태입니다.
