...
Stage 1 (infrastructure migration) is complete. Stage 2 (Lifecycle Controller) is in progress — the backend code is built (Phase 1) and its foundational infra (DynamoDB, IAM, SQS, ECR, Secrets Manager) is deployed (Phase 2). The frontend (Stage 3) is scaffolded.
...
1단계(인프라 마이그레이션)는 완료되었습니다. 2단계(Lifecycle Controller)가 진행 중입니다 — 백엔드 코드가 구현되었고(Phase 1), 기반 인프라(DynamoDB, IAM, SQS, ECR, Secrets Manager)가 배포되었습니다(Phase 2). 프론트엔드(3단계)는 스캐폴드 상태입니다.
