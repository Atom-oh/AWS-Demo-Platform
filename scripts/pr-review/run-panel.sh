...
# 리다이렉트, 파일이라 TTY 아님 → no-hang); Kiro 는 stdin 을 무시하므로 size-capped argv
# 텍스트로 직접 embed 한다(툴 미부여 — 아래 KIRO_DIFF_TEXT 주석 참조). timeout 백스톱 + 비대화형 플래그로 멈춤 방지. 셀이 비면
...
# 잔여 read 위험의 완화가 아니라 순수 defense-in-depth.
...
# fs_read + untrusted diff = 절대경로 read 유도 후 공개 PR 코멘트로 exfiltration 가능
# (CRITICAL, claude-code-usage-dashboard PR #4 리뷰에서 발견 -- 동일 lens×model matrix
...
# TOTAL_MODELS-1 과 비교했을 뿐 벤더 축이 아니었다. 이 repo는 벤더 3개(codex/kiro/
# claude-self) -- 벤더 하나가 완전히 죽어도 나머지 둘이 교차확인하므로 아직 severe 가
# 아니다; 벤더 2개(= 전체-1)가 죽어야 ≤1 vendor 라는 계약이 성립한다.
CODEX_DEAD=0
...
CLAUDE_SELF_DEAD=0
grep -qx "claude-self" "$WORK/degraded-models.txt" 2>/dev/null && CLAUDE_SELF_DEAD=1
DEAD_VENDORS=$((CODEX_DEAD + KIRO_ALL_DEAD + CLAUDE_SELF_DEAD))
if [ "$DEAD_VENDORS" -ge 2 ]; then
  echo "::error::coverage collapsed to ≤1 vendor (codex dead=$CODEX_DEAD, kiro fully dead=$KIRO_ALL_DEAD, claude-self dead=$CLAUDE_SELF_DEAD) — forcing VERDICT: FAIL, no cross-vendor check remains for any lens" >&2
