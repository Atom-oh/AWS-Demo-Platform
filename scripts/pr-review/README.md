...
The protocol and executors are staged; the live legacy workflow is unchanged.
`prepare_roles.py` validates pinned Git scope, `run_role.py` invokes specialists,
`synthesize_roles.py` enforces adjudication limits, and `restore_role_frames.py`
restores private request files after receipt-only CI transport. Activation needs
a separate reviewed change. `role_review.py` itself remains offline.
...
AWS Demo Platform already uses Sol; activation retains it. Terra is historical
(ADR-013). Its preparer must explicitly pass `--context-cap 12288`.
