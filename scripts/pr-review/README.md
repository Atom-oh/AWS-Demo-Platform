...
AWS Demo Platform retains its 12,288-byte context ceiling in the active workflow.
...
starts a new record lifecycle; wait for all writers before aggregating. Issuance
archives prior results in `slot/TAG-attempts.json` (up to 32 prior attempts).
Model-selection, fallback, quota and agent-preflight failures remain sticky for
the prepared review and block a later clean retry. Only a new preparation clears
this owned history; current upstream flags remain. Summaries include attempt history.
