...
- **Multi-AI PR review panel** ([ADR-007](decisions/ADR-007-multi-ai-pr-review-panel.md)) — `pr-review.yml` runs a Codex (Bedrock) + Kiro (`opus`/`kimi-k2.5`/`glm-5`) panel feeding a Claude Opus 4.8 chair that synthesizes one review + fail-closed `VERDICT`. Runner image (`actions-runner-claude`) built in-repo; Kiro auth via ExternalSecret.
