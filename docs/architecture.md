...
- **Multi-AI PR review panel** ([ADR-007](decisions/ADR-007-multi-ai-pr-review-panel.md)) — `pr-review.yml` runs a Codex (Bedrock) + Kiro (`claude-opus-4.8`/`kimi-k2.5`/`glm-5`) + Antigravity (`agy`, free-tier best-effort) panel feeding a Claude Opus 4.8 chair that synthesizes one review + fail-closed `VERDICT`. Runner image (`actions-runner-claude`) built in-repo; Kiro/Antigravity auth via ExternalSecret `ai-panel-keys` from Secrets Manager `/demo-platform/actions/AI-key`.
