...
- **Grafana administrator credential** — Phase A provisions Secrets Manager `/demo-platform/grafana/admin` and ESO `monitoring/grafana-admin` only. A separate phase B changes the Grafana Helm chart and sidecars to consume that Secret after ESO is Ready and the persisted `admin` password has been rotated and verified. A Secret update alone does not rotate the database credential.
