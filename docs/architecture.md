...
- **self-hosted 러너 fleet (hub)** — ARC(`gha-runner-scale-set`) ApplicationSet(`argocd-apps/system/appset-helm-runner-{arm,claude-arm,x86}-<repo>.yaml`)이 `mall-apne2-mgmt`의 `actions-runner-system` ns에 repo별 scale set을 등록합니다(arm/x86은 `github-app-secret`+DinD, claude-arm은 공유 `claude-runner` SA+`ai-panel-keys`). 내부 `AWS-Demo-Platform` 외에 외부 repo도 포함 — 예: `ai-trader-web`(scale set `ai-trader-web-arm`/`ai-trader-web-claude-arm`), Terraform은 워크플로 migration(후속 PR) 후 위 OIDC role을 assume ([ADR-012](decisions/ADR-012-ai-trader-web-oidc-plan-apply-split.md)).
