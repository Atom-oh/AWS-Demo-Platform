...
- **Grafana administrator credential** — Secrets Manager `/demo-platform/grafana/admin` supplies ESO `monitoring/grafana-admin`, consumed by the Grafana Helm chart and both provisioning sidecars. The consumer change follows a separate producer rollout with ESO Ready and persisted `admin` password verification. A Secret update alone does not rotate the database credential.
