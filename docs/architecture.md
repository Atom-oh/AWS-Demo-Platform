...
- **Multi-AI PR review panel** ([ADR-007](decisions/ADR-007-multi-ai-pr-review-panel.md), [ADR-011](decisions/ADR-011-pr-review-kiro-roster-gpt55-drop-v3.md), [ADR-013](decisions/ADR-013-pr-review-gpt56-model-bump.md), [ADR-014](decisions/ADR-014-pr-review-opus5-model-bump.md), [ADR-015](decisions/ADR-015-pr-review-per-model-parallel-jobs.md)) — `pr-review.yml` runs a lens×model matrix: Codex (Bedrock, `openai.gpt-5.6-sol`) + Kiro (`claude-opus-5`/`gpt-5.6-terra`/`glm-5`, via `kiro-cli chat` — no `--v3`) + a Claude self-review, each independently reviewing 4 lenses (L2–L5). Since ADR-015 this runs as **5 parallel `panel` jobs** (one per model, `pull-requests: read` only), each uploading its 4 cells as a GitHub Actions artifact; a separate `chair` job (`pull-requests: write`, `if: always()` so a total panel wipeout still fail-closes) downloads all 5 artifacts, judges coverage floors, and runs a Claude Fable 5 chair (fallback `claude-opus-5`) that synthesizes one lens-grouped review + fail-closed `VERDICT`. Runner image (`actions-runner-claude`) built in-repo; Kiro auth via ExternalSecret `ai-panel-keys` from Secrets Manager `/demo-platform/actions/AI-key`. Antigravity/`agy` is excluded because headless API-key auth does not work.
...
| `infra/iam` | DashboardEcsTaskRole-dev + ExecutionRole-dev + DemoPlatformOperator (2단계) + GHA OIDC 역할 (`demo-platform-gha-ecr-push`, `ai-trader-web-terraform-plan`/`-admin`, 편입된 `ai-trader-web-gha-deploy`) |
...
- **self-hosted 러너 fleet (hub)** — ARC(`gha-runner-scale-set`) ApplicationSet(`argocd-apps/system/appset-helm-runner-{arm,claude-arm,x86}-<repo>.yaml`)이 `mall-apne2-mgmt`의 `actions-runner-system` ns에 repo별 scale set을 등록합니다(모두 GitHub 인증에 `github-app-secret` 사용; arm/x86은 DinD 사이드카 추가, claude-arm은 공유 `claude-runner` SA+`ai-panel-keys` 추가). 내부 `AWS-Demo-Platform` 외에 외부 repo도 포함 — 예: `ai-trader-web`(scale set `ai-trader-web-arm`/`ai-trader-web-claude-arm`), Terraform은 워크플로 migration(후속 PR) 후 위 OIDC role을 assume ([ADR-012](decisions/ADR-012-ai-trader-web-oidc-plan-apply-split.md)).
