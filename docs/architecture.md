...
- **Grafana administrator credential** — Secrets Manager `/demo-platform/grafana/admin` → ESO `monitoring/grafana-admin` → the Grafana Helm chart and sidecars. The persisted Grafana administrator password must match that managed value; a Secret update alone does not rotate the database credential.
