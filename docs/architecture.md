...
- **Multi-AI PR review panel** ([ADR-007](decisions/ADR-007-multi-ai-pr-review-panel.md), [ADR-011](decisions/ADR-011-pr-review-kiro-roster-gpt55-drop-v3.md), Kiro diff delivery amended by [ADR-012](decisions/ADR-012-pr-review-kiro-diff-delivery-argv-embed.md)) — `pr-review.yml` runs a lens×model matrix: Codex (Bedrock) + Kiro (`claude-opus-4.8`/`gpt-5.5`/`glm-5`, via `kiro-cli chat` — no `--v3`) + a Claude self-review, each independently reviewing 4 lenses (L2–L5) — feeding a Claude Fable 5 chair that synthesizes one lens-grouped review + fail-closed `VERDICT`. Runner image (`actions-runner-claude`) built in-repo; Kiro auth via ExternalSecret `ai-panel-keys` from Secrets Manager `/demo-platform/actions/AI-key`. Antigravity/`agy` is excluded because headless API-key auth does not work. Kiro's diff is delivered as a capped, argv-embedded string with `--trust-tools=` (no tool grant) rather than a file-path read, closing a prompt-injection exfiltration path (ADR-012).
