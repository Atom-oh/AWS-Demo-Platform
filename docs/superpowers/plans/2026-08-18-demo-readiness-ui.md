...
- [ ] **Step 1 (TDD)**: Write a test for the revised `toggle()` in `useProjects.ts`, asserting it always resolves (never rejects) `{ok: true}` on `succeeded`, and `{ok: false}` on `failed`, on `partial_failure` (checked as soon as either status is observed, not deferred to the poll timeout), and on the poll loop exhausting its timeout while the job is still running.
...
- [ ] **Step 1**: Write the ADR: context (operators need to bump ArgoCD/HPA replica count or ECS `desiredCount` ahead of a demo, independent of on/off), decision (a new `scale` job operation that deliberately skips the state-transition invariant `turn_on`/`turn_off` use, with `targets` persisted on the job record, exposed via a route outside the existing `:op` pattern, a worker-side status recheck at the start of the branch, target-level failure aggregation, and a `HistoryClient` record for audit parity), and consequences — explicitly including all three accepted limitations from the spec's Known-limitations section (ArgoCD/HPA scaling is inert today pending the separate `namespace: 'placeholder'` fix, the irreversible HPA range collapse once that's fixed, and the narrowed-not-eliminated `turn_off` race) so they're recorded as a conscious trade-off, not just prose in a dated spec that will drift out of view.
