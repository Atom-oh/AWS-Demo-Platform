...
- [ ] **Step 1 (TDD)**: Write a test for the revised `toggle()` in `useProjects.ts`, asserting it always resolves (never rejects) `{ok: true}` on `succeeded`, and `{ok: false}` on `failed`, on `partial_failure` (checked as soon as either status is observed, not deferred to the poll timeout), on the poll loop exhausting its timeout while the job is still running, on the initial POST throwing/rejecting (e.g. a network error), and on the initial POST returning `409` — the last of these is not a hypothetical: `turnOnAll()`'s concurrency-limited batch can race two `toggle('turn_on')` calls against the same already-transitioning project, and the API's existing status-precondition check answers with `409`.
...
- [ ] **Step 5**: Add a "Turn on all" button in `app/page.tsx` near `StatStrip`; on click, call `turnOnAll()`, then toast a summary from its resolved results. The summary can only report `{ok: boolean}` per project, so a poll-timeout-exhausted project (still running, not actually failed) is unavoidably grouped with real failures in that summary — an accepted simplification, not a bug to chase in this pass.
...
- [ ] **Step 3 (TDD)**: Add a single exported `MAX_SCALE_REPLICAS` constant (e.g. `20`) to `packages/shared/src/step-key.ts` (or an equally shared module next to it) so `worker` and `api` validate against the same ceiling rather than two independently-chosen numbers. Write a failing `controllers/__tests__/ecs.test.ts` case for a new `setDesiredCount({cluster, service, count})` that calls `UpdateServiceCommand` with the given count and does not touch any restoration-data bookkeeping. Reject (throw) a non-positive count or one exceeding `MAX_SCALE_REPLICAS` before calling AWS at all — same validation, against the same constant, belongs on the ArgoCD side for `replicas`. Implement it.
...
- [ ] **Step 2 (TDD)**: Write a test for a new `scale(repo, targets)` function in `useProjects.ts` — same POST → job_id → 1s-interval poll pattern as `toggle()`, resolving/toasting `succeeded`/`failed`/`partial_failure` distinctly, with a reminder toast on `succeeded`/`partial_failure` matching the spec's Frontend components wording exactly: for an `ecs` target, "this becomes the new `turn_off` restore point" (accurate unconditionally, since ECS `desiredCount` has no min/max range to lose); for an `argocd-app` target, phrased *conditionally* rather than asserting range loss outright — e.g. "if this application contains an HPA, this pins its autoscaling range to a fixed count, which cannot be recovered through this tool afterward, even by scaling back down" — because the frontend only knows a target is `argocd-app` vs. `ecs`, never the underlying workload kind (Deployment/StatefulSet vs. HPA) within that application. On `partial_failure`, scope the reminder to only the targets the response reports as actually applied — a target that failed didn't take effect, so it gets no reminder about its (non-existent) new baseline.
- [ ] **Step 3**: Add `scaleProject` to `lib/api.ts` and implement `scale()` in `useProjects.ts`.
- [ ] **Step 4**: In `DetailDrawer.tsx`, for `ecs` resource chips, add an empty number input (placeholder: "check the console for the current count") and an "Apply" button, keyed by the resource's `stepKey` from the API response; disable both the input and the Apply button when project status isn't `on`. For `argocd-app` resource chips, disable the same input/button pair unconditionally and show an inline note that ArgoCD/HPA scaling doesn't work yet (see the spec's Known limitations) — shipping a live control that is documented to always fail is worse than not shipping it; re-enable it once the `namespace: 'placeholder'` bug is fixed separately.
...
- [ ] **Step 1**: Write the ADR following ADR-016's template sections (Status, Context, Options Considered, Decision, Consequences): context (operators need to bump ArgoCD/HPA replica count or ECS `desiredCount` ahead of a demo, independent of on/off), decision (a new `scale` job operation that deliberately skips the state-transition invariant `turn_on`/`turn_off` use, with `targets` persisted on the job record, exposed via a route outside the existing `:op` pattern, a worker-side status recheck at the start of the branch, target-level failure aggregation, HPA-before-workload patch ordering, and a `HistoryClient` record for audit parity), and consequences — explicitly including five accepted limitations so they're recorded as a conscious trade-off, not just prose in a dated spec that will drift out of view: (1) ArgoCD/HPA scaling is inert today pending the separate `namespace: 'placeholder'` fix — and note that this same bug already silently no-ops today's `turn_on`/`turn_off` ArgoCD path, not only the new `scale` feature; (2) the irreversible HPA range collapse once that bug is fixed; (3) the narrowed-not-eliminated `turn_off` race; (4) one replica value is broadcast to every workload handle on a multi-workload ArgoCD application, even though the spec itself notes "the current value" isn't well-defined per-application for that case; (5) the residual gap where a job stays orphaned `pending` if the enqueue-failure rollback update itself also fails. Also record that `scale`, like `turn_on`/`turn_off` before it, depends on the cluster-wide ArgoCD `ignoreDifferences` for Deployment/StatefulSet `/spec/replicas` and HPA `/spec/{min,max}Replicas` (`k8s/system/argocd/values.yaml`) — an Application outside that coverage would have a `scale` patch quietly reverted mid-demo.
