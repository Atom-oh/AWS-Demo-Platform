...
- Modify `packages/worker/src/controllers/argocd.ts` — add `scale(application, replicas)`: enumerate workload handles via the existing `listWorkloads`, then dispatch per handle by its `kind` (`patchReplicas` for a Deployment-kind handle, `patchHpaBounds` for an HPA-kind handle — mirroring how `turnOn`/`turnOff` already branch on kind).
...
- [ ] **Step 1**: Move the `stepKey` computation out of `job-runner.ts` into `packages/shared/src/step-key.ts` as a pure, exported function; update `job-runner.ts` to import it from there. No behavior change — this is a pure relocation, covered by the existing restoration tests continuing to pass unmodified.
...
- [ ] **Step 4 (TDD)**: Write failing `controllers/__tests__/argocd.test.ts` cases for a new `scale(application, replicas)` method covering (a) an application whose `listWorkloads` returns only Deployment-kind handles — `patchReplicas` called per handle, `patchHpaBounds` never called; (b) only HPA-kind handles — `patchHpaBounds` called, `patchReplicas` never called; (c) a mixed set of both kinds — each handle gets the call matching its own kind. Implement it by branching on `handle.kind`, the same way `turnOn`/`turnOff` already do.
