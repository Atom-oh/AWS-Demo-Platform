...
4. **hub cluster를 단독 정의·소유** — 시스템 컴포넌트(ArgoCD, Prometheus, ClickHouse, Grafana 등)는 이 repo의 `k8s/system/`. 테넌트 프로젝트(multi-region-architecture 포함)는 `argocd-apps/tenants/`에 root Application CR로 등록 (App-of-Apps). **scope는 ap-northeast-2 region**에 한정 — multi-region-architecture의 us-east-1, us-west-2 워크로드는 이 플랫폼 범위 외.
...
2. **AWS-Demo-Platform이 hub의 단독 owner.** hub의 ArgoCD가 multi-region-architecture를 포함한 외부 repo를 source로 sync (App-of-Apps). **단 sync 대상은 ap-northeast-2 spoke 클러스터(`eks-az-a`, `eks-az-c`)만**. US 클러스터(us-east-1, us-west-2)는 multi-region-architecture에서 자체 관리하거나 비활성 상태로 두며, 이 플랫폼은 관여하지 않음.
...
```
argocd/                              # 신규 — tenant 자식 Applications (ap-northeast-2만)
├── shared-agents-hub.yaml           # hub에 external-secrets/fluent-bit/otel-collector/keda 배포
├── shared-agents-az-a.yaml          # az-a spoke에 동일 배포
├── shared-agents-az-c.yaml          # az-c spoke에 동일 배포
├── workloads-apne2-az-a.yaml        # 워크로드 App (az-a)
└── workloads-apne2-az-c.yaml        # 워크로드 App (az-c)
```

AWS-Demo-Platform/argocd-apps/tenants/multi-region-mall.yaml이 이 디렉토리를 가리킴 (App-of-Apps).

**범위 제한**: us-east-1, us-west-2 클러스터의 워크로드 매니페스트는 multi-region-architecture에 잔류하나 이 플랫폼의 ArgoCD가 sync하지 않음. multi-region-arch가 자체 관리하거나 비활성 상태.
...
Step 5: Tenant 등록 + 공유 agents 재배포 (ap-northeast-2만)
  - multi-region-architecture에 argocd/ 디렉토리 PR (az-a, az-c용 파일만)
  - AWS-Demo-Platform/argocd-apps/tenants/multi-region-mall.yaml 등록
  - shared-agents가 hub + ap-northeast-2 spoke (az-a, az-c)에 배포됨
  - OTel-collector config: ClickHouse exporter로 변경 (Tempo exporter 제거)
  - fluent-bit output: ClickHouse로 변경
  - us-east-1, us-west-2 클러스터는 이 단계에서 미관리
...
- [ ] multi-region-mall의 ap-northeast-2 spoke 워크로드(az-a, az-c)가 hub ArgoCD에서 정상 sync (Healthy + Synced). us-east-1/us-west-2는 검증 대상 외
...
| OQ-3 | (해결됨) `external-secrets`, `fluent-bit`, `otel-collector`, `keda`는 워크로드 spoke에도 필요 → multi-region-arch에 잔류, hub + ap-northeast-2 spokes(az-a, az-c)에만 배포. us-east-1/us-west-2는 이 플랫폼 범위 외 | — | ✓ |
