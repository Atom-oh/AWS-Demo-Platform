# ADR-007: Multi-AI co-agent PR review panel (Codex + Kiro + Antigravity) with a Claude chair
...
`openai.gpt-5.5`, `us-east-2`) — no key, reusing the runner node IAM, whose
`ci_runner_bedrock` policy is already `Resource=*` (all regions). **Kiro and
Antigravity use `KIRO_API_KEY` / `ANTIGRAVITY_API_KEY`** read from the existing
Secrets Manager secret `actions/AI-key` via an `external-secrets.io/v1`
ExternalSecret (`ai-panel-keys`) into the runner pod env — no new slot.
**Antigravity is free-tier (rate-limited), so a best-effort panelist** that
skips cleanly on quota/429. The runner stays in **ap-northeast-2**
...
### Positive
- Cross-family review diversity (OpenAI gpt-5.5 + Kiro opus/kimi/glm + Antigravity) with one synthesized verdict.
...
- No new secret slot — reuses the existing `actions/AI-key` secret.

### Negative
- Up to 6 model calls per PR (latency; acceptable for non-prod async review).
- Chair is a single synthesis point; a bad chair run still fail-closes via the VERDICT rule.
- `kimi-k2.5` may be account-tier gated, and Antigravity is free-tier rate-limited → those silently skip.
...
`openai.gpt-5.5`, `us-east-2`)로 키 불필요, 러너 노드 IAM 재사용
(`ci_runner_bedrock`가 이미 `Resource=*`라 추가 IAM 불필요). **Kiro·Antigravity는
`KIRO_API_KEY`·`ANTIGRAVITY_API_KEY`** — 기존 Secrets Manager 시크릿
`actions/AI-key` → `external-secrets.io/v1` ExternalSecret(`ai-panel-keys`) → 러너
env (신규 슬롯 없음). **Antigravity는 free tier(rate-limited)라 best-effort** —
쿼터/429 시 깨끗이 skip. 러너는 **ap-northeast-2 유지**(크로스리전 Bedrock
...
### 긍정적
- 교차 패밀리 리뷰 다양성(OpenAI gpt-5.5 + Kiro opus/kimi/glm + Antigravity) + 단일 종합 VERDICT.
- 기존 게이트(fail-closed)·코멘트 upsert·concurrency 불변식 그대로.
- 러너 이미지와 빌드 파이프라인을 이 repo에서 소유·PR 리뷰.
- 신규 시크릿 슬롯 없음 — 기존 `actions/AI-key` 재사용.

### 부정적
- PR당 최대 6회 모델 호출(지연; 비프로덕션 비동기 리뷰에 허용).
- 의장 단일점이지만 의장 실패도 VERDICT 룰로 fail-closed.
- `kimi-k2.5`는 계정 등급 제한, Antigravity는 free tier rate-limit 가능 → 해당 항목 silent skip.
