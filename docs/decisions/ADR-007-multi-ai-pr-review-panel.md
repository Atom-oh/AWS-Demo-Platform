...
- **Base re-pointed to the official ARC runner image (self-referential FROM fixed)** —
  ...
  (Ubuntu 24.04 + .NET + runner agent) **via an ECR pull-through cache** (prefix `ghcr`)
  and installs only gh/aws/node/**claude-code**/codex/kiro-cli/plugins on top. The runner
  agent/OS/.NET come from upstream (no hand-rolled agent), and each weekly build is clean.
  Adds `infra/ecr/pull-through-cache.tf` (PTC rule + `ecr-pullthroughcache/ghcr` PAT slot,
  manual value) and grants the build role `ghcr/*` import + the previously-missing
  `actions-runner-claude` push. (`actions/runner-images` is VM images, not a container base.)
...
- **Kiro v3** — panel calls use `kiro-cli --v3 chat …` (binary is `kiro-cli`, never
  bare `kiro`); the build verifies `kiro-cli --v3 --help` so a missing/renamed flag
