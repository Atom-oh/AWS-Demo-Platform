...
- `demo-platform-gha-ecr-push` — GitHub Actions OIDC role (Phase 3); trust restricted to `repo:Atom-oh/AWS-Demo-Platform:ref:refs/heads/main`; ECR push perms on `demo-platform/{api,worker,frontend}`. Used by `.github/workflows/backend-ci.yml` (api/worker) and `frontend-ci.yml` (frontend) push jobs.
