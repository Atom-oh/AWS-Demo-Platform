...
- `demo-platform-gha-ecr-push` — GitHub Actions OIDC role (Phase 3); trust restricted to `repo:Atom-oh/AWS-Demo-Platform:ref:refs/heads/main`; ECR push perms on `demo-platform/{api,worker,frontend}` + `actions-runner-claude`, and `ghcr/*` pull-through-cache import (`BatchImportUpstreamImage`/`CreateRepository`) for the runner-image base. Used by `backend-ci.yml` (api/worker), `frontend-ci.yml` (frontend), and `runner-image.yml` (actions-runner-claude) push jobs.
