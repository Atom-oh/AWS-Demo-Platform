...
data "aws_network_interface" "use1_runtime" {
  provider = aws.use1
  count    = length(local.endpoint_subnets.use1)
  id       = tolist(aws_vpc_endpoint.use1_runtime.network_interface_ids)[count.index]
}

data "aws_network_interface" "use1_mantle" {
  provider = aws.use1
  count    = length(local.endpoint_subnets.use1)
  id       = tolist(aws_vpc_endpoint.use1_mantle.network_interface_ids)[count.index]
}

data "aws_network_interface" "use2_runtime" {
  provider = aws.use2
  count    = length(local.endpoint_subnets.use2)
  id       = tolist(aws_vpc_endpoint.use2_runtime.network_interface_ids)[count.index]
}

data "aws_network_interface" "use2_mantle" {
  provider = aws.use2
  count    = length(local.endpoint_subnets.use2)
  id       = tolist(aws_vpc_endpoint.use2_mantle.network_interface_ids)[count.index]
}
...
locals {
  phz = {
    use1_runtime = {
      ...
      ips  = data.aws_network_interface.use1_runtime[*].private_ip
    }
    use1_mantle = {
      zone = "bedrock-mantle.us-east-1.api.aws"
      ips  = data.aws_network_interface.use1_mantle[*].private_ip
    }
    use2_runtime = {
      zone = "bedrock-runtime.us-east-2.amazonaws.com"
      ips  = data.aws_network_interface.use2_runtime[*].private_ip
    }
    use2_mantle = {
      zone = "bedrock-mantle.us-east-2.api.aws"
      ips  = data.aws_network_interface.use2_mantle[*].private_ip
    }
  }
}
