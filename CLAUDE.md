...
- **AI PR review** — `pr-review.yml` on the `aws-demo-platform-claude-arm` runner: a multi-AI panel (Codex via Bedrock + Kiro `opus`/`kimi-k2.5`/`glm-5` + Antigravity `agy`) feeds a **Claude Opus 4.8 chair** that synthesizes one review + `VERDICT` (fail-closed gate). Orchestration in `scripts/pr-review/`. Runner image built in-repo (`docker/actions-runner-claude/` + `runner-image.yml`). Kiro/Antigravity auth via `KIRO_API_KEY`/`ANTIGRAVITY_API_KEY` from Secrets Manager `/demo-platform/actions/AI-key` (ExternalSecret `ai-panel-keys`); Antigravity is free-tier best-effort. See [ADR-007](docs/decisions/ADR-007-multi-ai-pr-review-panel.md).
