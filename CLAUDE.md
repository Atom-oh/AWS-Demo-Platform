...
- **AI PR review** — `pr-review.yml` on the `aws-demo-platform-claude-arm` runner: a lens×model matrix (L2–L5, each reviewed independently by Codex (`openai.gpt-5.6-sol`) via Bedrock + Kiro `claude-fable-5`/`gpt-5.6-sol` (tags `kiro-fable`/`kiro-sol`) via `kiro-cli chat` (no `--v3` — it routes to a narrower catalog that rejected `gpt-5.5`, ADR-011; `glm-5` dropped per ADR-015 — false-positive-prone) + a Claude self-review, 4 models × 4 lenses = 16 cells, English-only prompts/output) runs as **4 parallel `panel` jobs (one per model, matrix, `pull-requests: read` only, GH tokens scoped to the `claude-self` cell)**, each uploading its 4 lens cells as a single-directory artifact; a separate `chair` job (`pull-requests: write`, `if: !cancelled()`) resets its slot dir, downloads all artifacts (`continue-on-error`), aggregates coverage floors, and runs a **Claude Fable 5 chair** (fallback `claude-opus-5`) that synthesizes one lens-grouped review + `VERDICT` (fail-closed gate) — ADR-015 (per-model parallel jobs replacing the earlier single-job 20-process fan-out). Orchestration in `scripts/pr-review/` (`prepare-inputs.sh` → `run-panel.sh <model_tag>` → `aggregate.sh` → `synthesize.sh`). Runner image built in-repo (`docker/actions-runner-claude/` + `runner-image.yml`), **rebuilt weekly** (Sun 03:00 KST cron, best-effort) and shipping baked Claude Code plugins (`codex@openai-codex`, `code-review@`/`github@claude-plugins-official`, Codex plugin pre-configured). Kiro auth via `KIRO_API_KEY` from Secrets Manager `/demo-platform/actions/AI-key` (ExternalSecret `ai-panel-keys`). Antigravity/`agy` is intentionally absent because headless API-key auth does not work. **Runner pods use the shared `claude-runner` SA → `mall-apne2-mgmt-ci-runner` role (Bedrock + bedrock-mantle) via EKS Pod Identity; this SA must be in `infra/eks-mgmt` `runner_service_accounts` or codex loses bedrock-mantle creds.** See [ADR-007](docs/decisions/ADR-007-multi-ai-pr-review-panel.md), [ADR-011](docs/decisions/ADR-011-pr-review-kiro-roster-gpt55-drop-v3.md), [ADR-013](docs/decisions/ADR-013-pr-review-gpt56-model-bump.md), [ADR-014](docs/decisions/ADR-014-pr-review-opus5-model-bump.md), [ADR-015](docs/decisions/ADR-015-pr-review-per-model-parallel-jobs.md).
